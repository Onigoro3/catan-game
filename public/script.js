...
function resizeCanvas() {
    ...
    ORIGIN_X = canvas.width/2; ORIGIN_Y = canvas.height*(isMobile?0.4:0.5);
    const minDim = Math.min(canvas.width, canvas.height);
    // マップサイズ(settings.mapSize)に応じて調整
    const isExtended = gameState && gameState.settings && gameState.settings.mapSize === 'extended';
    const sf = isExtended ? 16 : 13;
    HEX_SIZE = Math.max(isMobile?32:45, minDim/sf);
    if(gameState) render();
}
...
function startGame() { 
    try { 
        // サーバーの設定(gameState.settings)に従ってマップ生成
        if (!gameState) return;
...
function createBoardData(mapSize, mapType) {
    ...
    if (mapSize === 'extended') baseNums = [...baseNums, 2,3,4,5,6,8,9,10,11,12]; // 追加
...
// ... 他の関数 (playDiceAnim, endTurn, sendTrade, buyCard, playCard, setBuildMode, etc...) は変更なし
...
function updateBurstUI() {
    ...
    Object.keys(p.resources).forEach(r => {
        if(p.resources[r] > 0) {
            html += `<div style="margin:5px;">${RESOURCE_INFO[r].icon}: <button onclick="burstDrop['${r}'] = Math.max(0, burstDrop['${r}']-1); updateBurstUI();">-</button> ${burstDrop[r]} / ${p.resources[r]} <button onclick="if(burstDrop['${r}'] < p.resources['${r}']) burstDrop['${r}']++; updateBurstUI();">+</button></div>`;
        }
    });
...
if(socket) {
    ...
    socket.on('gameStarted', s => { 
        ...
        // 部屋情報表示
        if(s.settings) document.getElementById('room-info-display').innerText = `設定: ${s.settings.humanLimit}人+Bot${s.settings.botCount}, ${s.settings.victoryPoints}点先取`;
        resizeCanvas(); render(); updateUI(); 
    });
    socket.on('updateState', s => { 
        gameState=s; 
        // 自分のIDを部屋IDとして認識させる簡易処理（本来はServerから送るべきだが）
        ...
        if(s.phase==='BURST' && s.burstPlayers.includes(myId) && document.getElementById('burst-overlay').style.display==='none') {
        ...
        if(s.phase==='GAME_OVER') {
            ...
            let h="<h3>結果</h3>";
            h+=Object.keys(s.stats.resourceCollected).map(id=>{const p=s.players.find(pl=>pl.id===id);return `${p.name}: ${s.stats.resourceCollected[id]}枚`;}).join('<br>');
    ...
    socket.on('chatUpdate', d => {
        const b=document.getElementById('chat-messages'); const p=document.createElement('div');
        p.style.fontSize='11px'; p.innerHTML=`<span style="color:${d.color}">${d.name}</span>:${d.msg}`;
...
function updateUI() {
    ...
    // 既存の選択肢を保持しつつ更新するのは面倒なので、毎回リセットし再構築（銀行とBot以外）
    const currentVal = sel.value;
    sel.innerHTML = '<option value="bank">銀行</option><option value="bot">Bot</option>';
    gameState.players.forEach(pl => {
        if(pl.id !== myId && !pl.isBot) { 
            const opt = document.createElement('option'); opt.value = pl.id; opt.innerText = pl.name; 
            sel.appendChild(opt); 
        }
    });
    ...
    const scoreHTML=gameState.players.map(p=>`<div>${p.name}: ${p.victoryPoints}</div>`).join('');
    ...
    if(isMobile) {
        ...
        document.getElementById('pc-timer').innerText = gameState.timer;
    }
    ...
    if(gameState.phase==='MAIN'&&cur.id===myId) {
        ...
        mainCtrl.style.display='none'; document.getElementById('action-msg').innerText="盗賊を移動させてください";
    } else if(gameState.phase==='BURST'&&gameState.burstPlayers.includes(myId)) {
        mainCtrl.style.display='none'; document.getElementById('action-msg').innerText="手札過多: 資源を捨ててください";
    } else {
        mainCtrl.style.display='none'; document.getElementById('action-msg').innerText="待機中...";
    }
}

...
let isDragging=false, lastPointer={x:0,y:0}, lastPinchDist=0;
...
function resizeCanvas() {
    ...
    // UIと被らないよう中心位置を調整
    ORIGIN_Y = canvas.height * (isMobile ? 0.5 : 0.5); 
    
    const minDim = Math.min(canvas.width, canvas.height);
    const scaleFactor = (gameState && gameState.settings && gameState.settings.mapSize==='extended') ? 16 : 13;
    
    // スマホのときはデフォルトで少し大きく表示
    const baseSize = Math.max(isMobile ? 38 : 45, minDim/scaleFactor);
    HEX_SIZE = baseSize;
...
function createBoardData(mapSize, mapType) {
    ...
    if(mapSize==='extended') resList.push('desert'); // 拡張なら砂漠2つ
    ...
    hexes.forEach(h=>{ 
        ...
        if(h.resource==='desert') h.number=0; 
        else h.number=nums[n_idx++]||7; 
    });
...
if(socket) {
    socket.on('connect', () => { 
        ...
        if(s){s.innerText="🟢 接続OK"; s.style.color="green"; document.getElementById('join-btn').disabled=false;} 
    });
    
    socket.on('disconnect', () => { 
        const s=document.getElementById('connection-status'); 
        if(s){s.innerText="🔴 切断"; s.style.color="red"; document.getElementById('join-btn').disabled=true;} 
    });
    ...
    socket.on('chatUpdate', d => {
        ...
        p.innerHTML=`<span style="color:${d.color}">${d.name}</span>:${d.msg}`;
...
function render() {
    ...
    hexes.forEach(h => {
        ...
        if (currentHexSize > 15) {
            ...
            ctx.font = `${currentHexSize*0.5}px Arial`; 
            ctx.fillText(RESOURCE_INFO[h.resource].icon, p.x, p.y-currentHexSize*0.3);
            
            ctx.font = `bold ${currentHexSize*0.25}px Arial`; 
            ctx.fillText(RESOURCE_INFO[h.resource].label, p.x, p.y+currentHexSize*0.3);
            
            ctx.shadowBlur = 0;
            
            // ★数字トークン描画 (砂漠以外)
            if (h.resource !== 'desert' && h.number !== 0) {
                drawNumberToken(p.x, p.y, h.number, currentHexSize);
            }
        }
    ...
    if(ports) ports.forEach(p=>{
        ...
        if(v1&&v2){
            ...
            if(currentHexSize>10){
                ...
                else { ctx.fillText(RESOURCE_INFO[p.type].icon,pp.x,pp.y-currentHexSize*0.08); ctx.fillText('2:1',pp.x,pp.y+currentHexSize*0.1); }
            }
        }
    });
    ...
    edges.forEach(e=>{
        ...
        if(v1&&v2){ 
            ...
            if(e.owner) drawRoad(p1.x,p1.y,p2.x,p2.y,e.owner,currentHexSize); 
                ...
                ctx.strokeStyle='rgba(255,255,255,0.3)'; ctx.lineWidth=currentHexSize*0.08; ctx.stroke(); 
            } 
        }
    });
    ...
    vertices.forEach(v=>{ 
        ...
        if(v.owner) { 
            if(v.type==='city') drawCity(p.x,p.y,v.owner,currentHexSize); 
            else drawSettlement(p.x,p.y,v.owner,currentHexSize); 
        } else {
            ctx.fillStyle='rgba(255,255,255,0.5)'; ctx.beginPath(); ctx.arc(p.x,p.y,currentHexSize*0.1,0,Math.PI*2); ctx.fill();
        }
    });
}
...
function drawSettlement(x,y,c,s){
    const w=s*0.15; ctx.beginPath(); ctx.rect(x-w,y-w,w*2,w*2); 
    ctx.fillStyle=c; ctx.fill(); ctx.stroke();
}
function drawCity(x,y,c,s){
    const w=s*0.2; ctx.beginPath(); ctx.arc(x,y,w,0,Math.PI*2); 
    ctx.fillStyle=c; ctx.fill(); 

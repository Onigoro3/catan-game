...
function resizeCanvas() {
    ...
    const scaleFactor = isExtended ? 16 : 12; // 3-4-5-4-3なら大きく表示
...
if(socket) {
    ...
    socket.on('gameStarted', (state) => {
        ...
        if(state.settings) {
            document.getElementById('room-info-display').innerText = 
                `設定: 人間${state.settings.humanLimit}人 + Bot${state.settings.botCount}体`;
        }
    ...
    socket.on('updateState', (state) => {
        ...
        if (me) {
            ...
            // マップデータが存在すれば「ゲーム中」とみなす
            const isGameRunning = (state.board && state.board.hexes && state.board.hexes.length > 0);
            ...
            if (!isGameRunning) {
                ...
                btn.disabled = false; // 人数不足でもBotで埋めるので常に押せるようにする
            } else {
                // マップがある = ゲーム中
                document.getElementById('start-overlay').style.display = 'none';
                document.getElementById('controls').style.display = 'block';
        ...
        if (state.phase === 'GAME_OVER') {
            ...
            h += "<div>💰 資源:<br>" + Object.keys(state.stats.resourceCollected).map(pid=>{
...
function startGame() {
    if(!socket) return;
    socket.emit('startGame'); // サーバーに開始命令を送るだけ
    const btn = document.getElementById('start-btn-big');
...
function updateBuildMsg() {
    const m = !buildMode ? "" : (buildMode==='road'?"道":buildMode==='settlement'?"開拓":buildMode==='city'?"都市":"");
    document.getElementById('pc-build-msg').innerText = m;
    // スマホ用メッセージエリアがあれば表示（現在はalert/statusで代用）
}
...
function render() {
    ...
    hexes.forEach(h => {
        ...
        if(s > 15) {
            ...
            ctx.shadowBlur = 0;
            // 数字 (砂漠以外かつ0以外)
            if(h.resource !== 'desert' && h.number !== 0) drawNumberToken(p.x, p.y, h.number, s);
        }
        // 盗賊
...
function drawNumberToken(x,y,n,s) {
    ctx.fillStyle='rgba(255,255,255,0.9)'; ctx.beginPath(); ctx.arc(x,y,s*0.3, 0, Math.PI*2); ctx.fill(); ctx.strokeStyle='#333'; ctx.lineWidth=1; ctx.stroke();
    ctx.fillStyle=(n===6||n===8)?'#D32F2F':'black'; ctx.font=`bold ${s*0.25}px Arial`; ctx.textAlign='center'; ctx.textBaseline='middle'; ctx.fillText(n,x,y);
...
function updateUI() {
    ...
    if(gameState.phase==='MAIN' && cur.id===myId) {
        ...
        controls.style.display='none'; msgEl.innerText="盗賊を移動させてください";
    } else if(gameState.phase==='BURST' && gameState.burstPlayers.includes(myId)) {
        controls.style.display='none'; msgEl.innerText="資源を捨ててください";
    } else if(gameState.phase==='SETUP' && cur.id===myId) {
        controls.style.display='none'; msgEl.innerText = (gameState.subPhase==='SETTLEMENT') ? "開拓地を置いてください" : "道を置いてください";
    } else {
        controls.style.display='none'; msgEl.innerText=`待機中 (${cur.name}の手番)`;
    }
}
...
canvas.addEventListener('click', e => {
    ...
    const wx = (e.clientX - rect.left - camera.x) / (HEX_SIZE * camera.zoom);
    const wy = (e.clientY - rect.top - camera.y) / (HEX_SIZE * camera.zoom);

    if(gameState.phase === 'ROBBER') {
        let tH=null, minD=1.0;
        gameState.board.hexes.forEach(h=>{ const d=Math.hypot(h.x-wx, h.y-wy); if(d<minD){minD=d; tH=h;} });
    ...
    if(gameState.phase==='SETUP' || (gameState.phase==='MAIN' && gameState.diceResult)) {
        ...
        if(gameState.phase==='SETUP' || buildMode==='settlement' || buildMode==='city') {
            ...
            gameState.board.vertices.forEach(v=>{ const d=Math.hypot(v.x-wx, v.y-wy); if(d<minD){minD=d; tV=v;} });
            if(tV) {

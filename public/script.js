...
window.onload = function() {
    ...
    if(params.get('room')) {
        const roomNameInput = document.getElementById('join-roomname');
        if(roomNameInput) roomNameInput.value = params.get('room');
        showTab('join');
    } else {
        showTab('join');
    }
...
function resizeCanvas() {
    ...
    const scaleFactor = isExtended ? 16 : 13;
...
function playSystemSound(type) {
    const volElem = document.getElementById('pc-volume');
    const vol = volElem ? volElem.value : 0.3;
    if (vol <= 0) return;
...
if(socket) {
    socket.on('connect', () => {
        ...
        if(s) { s.innerText = "🟢 接続完了"; s.style.color = "green"; }
    ...
    socket.on('gameStarted', (state) => {
        ...
        if (state.settings) {
            const info = document.getElementById('room-info-display');
            if(info) info.innerText = `設定: 人間${state.settings.humanLimit}人 + Bot${state.settings.botCount}`;
        }
    ...
    socket.on('updateState', (state) => {
        ...
        // 部屋名補完
        if(!gameState.roomId) {
            const jr = document.getElementById('join-roomname');
            if(jr && jr.value) gameState.roomId = jr.value;
        }
...
function updateBuildMsg() {
    ...
    const pcMsg = document.getElementById('pc-build-msg');
    if(pcMsg) pcMsg.innerText = m;
    // スマホ用はアラートかアクションメッセージで代用
    const actMsg = document.getElementById('action-msg');
    if(actMsg && buildMode) actMsg.innerText = m;
}
    ...
    if(ports) ports.forEach(p => {
        ...
        if(v1 && v2) {
            ...
            if(s > 10) {
                ...
                ctx.fillStyle = 'black'; ctx.font = `${s*0.15}px Arial`; ctx.shadowBlur = 0;
...
canvas.addEventListener('click', e => {
    ...
    if(gameState.phase === 'ROBBER') {
        ...
        gameState.board.hexes.forEach(h => {
            ...
            // 半径(HEX_SIZE*zoom)より内側ならヒット
            if (dist < HEX_SIZE * camera.zoom && dist < minD) {
                minD = dist; tH = h;
            }
        });
    ...
    if(gameState.phase==='SETUP' || (gameState.phase==='MAIN' && gameState.diceResult)) {
        ...
        if(gameState.phase==='SETUP' || buildMode==='settlement' || buildMode==='city') {
            let tV=null, minD=30; // 30px以内なら反応

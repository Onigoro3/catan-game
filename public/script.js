...
function showTab(tab) {
    ...
    document.getElementById('tab-join').style.background = tab==='join' ? '#ddd' : 'white';
    document.getElementById('tab-create').style.background = tab==='create' ? '#ddd' : 'white';
}
...
const SKINS={normal:{bg:'#87CEEB',hex:'#fff',text:'#000'},dark:{bg:'#222',hex:'#444',text:'#fff'},retro:{bg:'#d2b48c',hex:'#f4a460',text:'#4b0082'}};
...
function resizeCanvas() {
    ...
    ORIGIN_Y = canvas.height * (isMobile ? 0.4 : 0.5);
    const minDim = Math.min(canvas.width, canvas.height);
    const sf = (gameState&&gameState.settings.mapSize==='extended')?16:13;
...
// UI Actions
function createRoom() {
    const name=document.getElementById('username').value, room=document.getElementById('create-roomname').value;
...
function joinGame() {
    const name=document.getElementById('username').value, room=document.getElementById('join-roomname').value;
    if(!name)return alert('名前入力');
    if(!socket||!socket.connected)return alert('接続中');
...
function startGame() { 
    try { 
        if(!gameState) return;
        const data=createBoardData(gameState.settings.mapSize, gameState.settings.mapType); 
        if(socket){ socket.emit('startGame', data); document.getElementById('start-btn-big').innerText="開始中..."; document.getElementById('start-btn-big').disabled=true; } 
    } catch(e){ alert(e); } 
}
...
function createBoardData(mapSize, mapType) {
    ...
    const numList=[]; let ni=0; while(numList.length<count){numList.push(baseNums[ni%baseNums.length]);ni++;} const nums=numList.sort(()=>Math.random()-0.5);
    let ri=0,n_idx=0; hexes.forEach(h=>{h.resource=res[ri++];if(h.resource==='desert')h.number=0;else h.number=nums[n_idx++];});
    ...
    const outer=vertices.filter(v=>Math.hypot(v.x-cx,v.y-cy)>(mapType==='random'?2.0:(mapSize==='extended'?3.2:2.4))).sort((a,b)=>Math.atan2(a.y-cy,a.x-cx)-Math.atan2(b.y-cy,b.x-cx));
    const portTypes=['any','pasture','any','forest','any','hill','any','field','mountain','any','any'];
    let pi=0; for(let i=0;i<outer.length&&pi<portTypes.length;i+=3){if(i+1<outer.length){const mx=(outer[i].x+outer[i+1].x)/2,my=(outer[i].y+outer[i+1].y)/2,ang=Math.atan2(my-cy,mx-cx);ports.push({type:portTypes[pi++],v1:outer[i].id,v2:outer[i+1].id,x:mx+0.4*Math.cos(ang),y:my+0.4*Math.sin(ang)});}}
...
if(socket) {
    ...
    socket.on('gameStarted', s => { gameState=s; document.getElementById('start-overlay').style.display='none'; document.getElementById('controls').style.display='block'; if(s.settings)document.getElementById('room-info-display').innerText=`設定: ${s.settings.humanLimit}人+Bot${s.settings.botCount}`; resizeCanvas(); render(); updateUI(); });
    socket.on('updateState', s => { 
        ...
        if(s.phase==='BURST' && s.burstPlayers.includes(myId) && document.getElementById('burst-overlay').style.display==='none') { burstTargetCount=Math.floor(Object.values(me.resources).reduce((a,b)=>a+b,0)/2); burstDrop={forest:0,hill:0,mountain:0,field:0,pasture:0}; updateBurstUI(); document.getElementById('burst-overlay').style.display='flex'; }
        if(s.phase==='GAME_OVER') { document.getElementById('winner-name').innerText=s.winner.name; let h="<h3>結果</h3>"; h+=Object.keys(s.stats.resourceCollected).map(id=>{const p=s.players.find(pl=>pl.id===id);return `${p.name}: ${s.stats.resourceCollected[id]}枚`;}).join('<br>'); document.getElementById('result-stats').innerHTML=h; document.getElementById('winner-overlay').style.display='flex'; }
        render(); updateUI(); 
    });
...
function render(){
    ...
    const skin = SKINS[currentSkin]; ctx.clearRect(0,0,canvas.width,canvas.height); ctx.fillStyle=skin.bg; ctx.fillRect(0,0,canvas.width,canvas.height);
    ...
    hexes.forEach(h=>{ const p=tr(h.x,h.y); drawHexBase(p.x,p.y,s,RESOURCE_INFO[h.resource].color); if(s>15){ ctx.fillStyle='white'; ctx.textAlign='center'; ctx.textBaseline='middle'; ctx.font=`${s*0.5}px Arial`; ctx.fillText(RESOURCE_INFO[h.resource].icon,p.x,p.y-s*0.3); ctx.font=`bold ${s*0.25}px Arial`; ctx.fillText(RESOURCE_INFO[h.resource].label,p.x,p.y+s*0.3); drawNumberToken(p.x,p.y,h.number,s); } if(gameState.robberHexId===h.id)drawRobber(p.x,p.y,s); if(gameState.phase==='ROBBER'&&gameState.players[gameState.turnIndex].id===myId){ctx.strokeStyle='red';ctx.lineWidth=3;ctx.stroke();} });
    if(ports)ports.forEach(p=>{ const v1=vertices.find(v=>v.id===p.v1),v2=vertices.find(v=>v.id===p.v2); if(v1&&v2){ const pp=tr(p.x,p.y),p1=tr(v1.x,v1.y),p2=tr(v2.x,v2.y); ctx.beginPath();ctx.moveTo(p1.x,p1.y);ctx.lineTo(pp.x,pp.y);ctx.lineTo(p2.x,p2.y);ctx.strokeStyle='#8B4513';ctx.lineWidth=s*0.08;ctx.stroke(); if(s>10){ctx.fillStyle='white';ctx.beginPath();ctx.arc(pp.x,pp.y,s*0.25,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.fillStyle='black';ctx.font=`${s*0.15}px Arial`; if(p.type==='any') ctx.fillText('3:1',pp.x,pp.y); else { ctx.fillText(RESOURCE_INFO[p.type].icon,pp.x,pp.y-s*0.08); ctx.fillText('2:1',pp.x,pp.y+s*0.1); }} } });
    edges.forEach(e=>{ const v1=vertices.find(v=>v.id===e.v1),v2=vertices.find(v=>v.id===e.v2); if(v1&&v2){ const p1=tr(v1.x,v1.y),p2=tr(v2.x,v2.y); if(e.owner)drawRoad(p1.x,p1.y,p2.x,p2.y,e.owner,s); else{ctx.beginPath();ctx.moveTo(p1.x,p1.y);ctx.lineTo(p2.x,p2.y);ctx.strokeStyle='rgba(255,255,255,0.3)';ctx.lineWidth=s*0.08;ctx.stroke();} } });
...
function drawNumberToken(x,y,n,s){if(n===null)return;ctx.fillStyle='rgba(255,255,255,0.9)';ctx.beginPath();ctx.arc(x,y,s*0.3,0,Math.PI*2);ctx.fill();ctx.strokeStyle='#333';ctx.lineWidth=1;ctx.stroke();ctx.fillStyle=(n===6||n===8)?'#D32F2F':'black';ctx.font=`bold ${s*0.25}px Arial`;ctx.fillText(n,x,y);}
...
function updateUI() {
    ...
    if(sel.options.length <= 2) { 
        gameState.players.forEach(pl => { if(pl.id!==myId&&!pl.isBot){ const opt=document.createElement('option'); opt.value=pl.id; opt.innerText=pl.name; sel.appendChild(opt); } });
    }
    ...
    const scoreHTML=gameState.players.map(pl=>`<div>${pl.name}: ${pl.victoryPoints}</div>`).join('');
    ...
    if(isMobile){
        ...
        document.getElementById('pc-log-area').innerHTML=logsHTML;
